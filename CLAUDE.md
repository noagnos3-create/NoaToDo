...
**Phase 7 is DONE (2026-07-17):** the export is two-step (scope: current list via `export_list` or all lists via `export_all`, then format `md`/`txt`; JSON dropped, N11.1.5) behind a pill next to the rail, the backend shows a real save dialog and writes the file (gate G21: sanitized filename suggestions, one-line tasks, UTF-8 no BOM + CRLF, cancel -> silent `canceled`, one native dialog at a time via `_native_dialog()` -> `busy`); `move_task(id, target_list_id)` and `reorder_lists(ordered_ids)` exist with the N11.2.2 edge cases (sidebar drag and drop, task drag onto a sidebar entry, right-click "Move to…" menu); list deletion has the Undo toast (N11.2.1, RAM buffer); gates G20, G21, G22, G29, G12 and G23 are all done. **Still an empty 1-line stub:** `backend/security.py`, i.e. the lock/panic + dual-layer-encryption work (Phase 8) is **not yet built**; Phase 8 starts with the Zweitprofil-Spike (U3, N11.8.3) and reading B.10 (G30).
