...
| `get_status()` | (keine) | `{ db, encryption, bitlocker, runtime, app, errors }` | Daten für das „App status"-Modal, seit Phase 8 durchweg real (G22): `db` = Tresordatei + Größe, `encryption` = beide Schichten, Argon2-Parameter, `pepper`, `last_wrap` (Mtime von `tasks.db.enc`, das ist genau der letzte Wrap), `active`; `bitlocker` = realer WMI-Status oder ehrlich `unknown` (G31); `app` = `{version, build_date, commit, frozen, signed, source}` aus `src/buildinfo.py` (Phase 9 / V10: dieselben Werte wie in den `.exe`-Ressourcen; `signed` sagt ehrlich, ob diese Fassung signiert ist, `source` nennt die Bezugsquelle, weil es **keinen** Update-Check übers Netz gibt); `errors` = redigierter G29-Ringpuffer. Das Modal rendert ausschliesslich diese Werte |
...
| `choose_vault_dir()` | (keine) | `{ path:str, has_vault:bool, warning:str\|null, bitlocker:{state:'protected'\|'off'\|'unknown', drive:str} }` oder `{ error:'canceled' }` | Onboarding-Schritt 1: öffnet den **nativen Ordner-Dialog** im Backend (`create_file_dialog`, FOLDER_DIALOG) und gibt den gewählten Ordner zurück. Prüft Schreibbarkeit und warnt bei Cloud-Sync-Pfaden (OneDrive/Dropbox, G32). **Liegt im gewählten Ordner schon eine `tasks.db.enc`, meldet er `has_vault:true`; das Onboarding bietet dann NICHT „neuen Tresor anlegen" an, sondern nur „diesen Tresor öffnen" (Pfad in `config.json` schreiben, dann Lock-Screen), damit ein bestehender Tresor nie überschrieben wird (N11.15.6).** Legt **nichts** an. **`bitlocker`** ist der reale Schutzstatus des gewählten Laufwerks (WMI, G31 (a)); die UI zeigt daraus die BitLocker-Empfehlung, `unknown` bleibt ein ehrliches „nicht lesbar" und wird nie als „aus" formuliert (G22). Im Onboarding erlaubt |
| `create_vault(path, passphrase)` | `str,str` | `{ ok:true, legacy_removed:bool }` | Onboarding-Schritt 2: legt den leeren Tresor an: Passphrase prüfen (**nur** Mindestlänge 12, N11.3, sonst `invalid`), 32-Byte-Pepper erzeugen und im Credential Manager ablegen (G18), Salt + Argon2-Parameter erzeugen, Schlüssel ableiten (G15), leere DB anlegen (nur Default-Settings, keine Demo-Daten, N11.1.4), als `tasks.db.enc` unter `path` schreiben (G16, atomar über `.tmp` + `os.replace`) und den Pfad in `config.json` speichern. **Bricht mit `invalid` ab, falls unter `path` schon eine `tasks.db.enc` liegt: ein bestehender Tresor wird NIE überschrieben (Datenverlust-Schutz, N11.15.6). Diesen Fall fängt regulär schon `choose_vault_dir()` ab (`has_vault:true`); der Backend-Riegel ist die letzte Sicherung gegen einen Aufruf an der UI vorbei.** Räumt dabei eine alte Dev-DB über den Secure-Delete-Pfad weg und meldet über **`legacy_removed`**, ob wirklich etwas gefunden wurde; nur dann zeigt die UI den quittierpflichtigen G33-Einmal-Hinweis (Onboarding-Schritt 4). Danach ist die App **entsperrt**. Im Onboarding erlaubt |
...
(`src/lockwindow.py`, Fallback aus N11.18) und kleine **Hinweisfenster** (z. B. die
...
4. **Eine Umsetzung: `src/wintheme.py`.** Dort liegen Tokens, DWM-Titelleiste,
...
1. **Gleiches Bild, keine zweite Optik.** Die Blende zeigt denselben Hintergrund
   ...
   die Entsperr-Animation N11.22 an) und nur Hintergrund (Weg ins Onboarding **und**
   Weg in die App beim normalen Programmstart, wo das Schloss bewusst zubleibt und
   die Logo-Blende die Begrüssung übernimmt, N11.26).
...
5. **Der Wechsel selbst ist unsichtbar** (Nutzerwunsch 2026-08-08: „das sieht immer
   ...
   Nähten sichtbar. Alle vier sind verbindlich geschlossen:
   - a) **Keine Windows-Fensteranimation.** Jedes eigene Fenster (WebView-Fenster,
     Sperrfenster, Blende) setzt `DWMWA_TRANSITIONS_FORCEDISABLED`; sonst zieht sich
     das alte Fenster unter der Blende sichtbar zusammen und das neue schiebt sich
     darunter wieder auf. Dieser Schalter und die Titelleistenfarbe werden **vor**
     dem ersten Anzeigen gesetzt (`HandleCreated`, nicht `Shown`), sonst blitzt für
     einen Moment eine helle Windows-Titelleiste auf.
   - b) **Gleiche Titelleiste, gleicher Taskleisten-Eintrag.** Die Blende trägt
     dieselben drei Fensterknöpfe wie die echten Fenster (ohne sie verschwindet das
     Knopftrio bei jedem Wechsel und kommt danach zurück) und dieselbe
     Taskleisten-Identität (AppID + Logo; ohne sie erscheint während jeder Übergabe
     kurz ein zweiter Eintrag mit dem generischen Python-Symbol). Bedienbar wird sie
     dadurch **nicht**: Schliessen und Minimieren werden abgefangen, der Zeitwächter
     aus Punkt 4 bleibt der einzige Notausgang.
   - c) **Gleiches Raster, gleiche Ringlage in allen drei Ansichten.** Das gemalte
     28px-Raster skaliert mit der Bildschirm-DPI: nativ wird in physischen Pixeln
     gemalt, im WebView in CSS-Pixeln, ungeskaliert wäre das native Raster auf einem
     150%-Bildschirm nur zwei Drittel so weit und die Rasterdichte änderte sich
     sichtbar mitten im Wechsel. Und das **WebView-Cover** (`renderLock`, die kurze
     Sperr-Blende in der App) setzt seinen Ring auf dieselbe Höhe wie die gemeinsame
     Ring-Geometrie in `lockwindow.py`; die beiden Masse sind ein Paar und werden nur
     zusammen geändert.
   - d) **Weiches Wegnehmen, fertiges Bild darunter.** Die Blende blendet in ~160 ms
     aus statt schlagartig zu verschwinden, meldet sich selbst erst dann als
     „steht", wenn sie wirklich gemalt hat (nicht schon beim Anzeigen), und WebView2
     wird angewiesen, auch **verdeckt** weiterzuzeichnen
     (`--disable-features=CalculateNativeWindowOcclusion`): Chromium hält ein
     vollständig überdecktes Fenster sonst für unsichtbar und liefert keine Bilder,
     sodass nach dem Wegnehmen der Blende kurz eine leere Fläche statt der fertigen
     App stünde. Reiner Darstellungsschalter, ohne Sicherheitswirkung.
...
- **LockScreen**, Vollbild über allem: Akzent-Ring mit Schloss und darunter die
  ...
  von vorn.
- **OnboardingScreens** (Phase 8, N11.13), Vollbild, drei Schritte (plus den vierten,
  der nur bei entsorgter Dev-Altdatei erscheint, G33), kein Header/keine
...
**Onboarding: die drei Screens plus der bedingte vierte (verbindlich, N11.13; Schritt 4 aus G33, ergänzt 2026-08-08)**
...
1. **Willkommen und Ort wählen.** Kurzer, ehrlicher Text: NoaToDo ist ein lokaler,
   ...
   Auf demselben Screen steht die **BitLocker-Empfehlung** (G31 (a), B.10.4 Punkt 1),
   sobald ein Ort gewählt ist, mit dem **realen** Status des gewählten Laufwerks aus
   `choose_vault_dir().bitlocker`. Drei Fassungen, weil ein gemessenes „aus" und ein
   nicht lesbarer Status nicht dasselbe behaupten dürfen (G22): `protected` bestätigt,
   dass die Kanäle gedeckt sind, die die App selbst nicht erreicht (Pagefile,
   Ruhezustandsdatei, Crash-Dumps); `off` empfiehlt nachdrücklich das Einschalten und
   nennt genau diese Kanäle; `unknown` sagt ausdrücklich, dass NoaToDo es **nicht weiss**
   (die Abfrage braucht meist Adminrechte) und bittet den Nutzer, selbst nachzusehen.
   Empfehlung, nie Sperre: weitergehen ist immer möglich.
...
4. **Nur bei Bedarf: der G33-Einmal-Hinweis.** Hat `create_vault()` wirklich eine alte
   Dev-DB gefunden und über den Secure-Delete-Pfad entsorgt (`legacy_removed:true`),
   kommt vor der Willkommens-Blende (N11.20) ein zusätzlicher, **quittierpflichtiger**
   Schritt („I have read this"), damit der Nutzer die forensische Restgrenze bewusst
   gelesen hat (so verlangt es G33). Er nennt beide Restkanäle: das Überschreiben ist auf
   SSDs mit Wear-Leveling **Best-Effort und keine Garantie**, und alte Export-Dateien aus
   der Dev-Zeit sind eigene Dateien des Nutzers, die die App bewusst **nicht** sucht und
   **nicht** löscht; wer das ausschliessen muss, braucht ein frisches, vollverschlüsseltes
   System (G31/B.10.4). Ohne Fund erscheint dieser Schritt nicht (ein Hinweis auf Vorrat
   wäre eine Behauptung ins Blaue), und er erscheint genau einmal, weil die alte DB danach
   weg ist.
...
- **Wann.** Nach einem erfolgreichen `create_vault()` (Onboarding-Schritt 3) und bei
  **jedem normalen Programmstart**. **Nicht** nach dem Entsperren aus einer laufenden
  ...
  Die Gegenprobe steht im Sperrschirm-Kapitel (N11.26): beim normalen Start geht dafür
  das Schloss **nicht** auf, sonst liefen zwei Begrüssungen hintereinander.
...
- **Der Sperrschirm trägt keine Textzeile (verbindlich, Etikett N11.26,
  ...
  passphrase and press Enter.") stehen dort noch; sichtbar sind nur der Ring und die
  Passwort-Pille, und die ist **kleiner** als zuvor (Nutzerwunsch). Beides sagte
  dasselbe ein zweites Mal: das Schloss zeigt den Zustand, der Platzhalter
  „Password" die Aufgabe, und Enter ist in einem Passwortfeld die gelernte Geste.
  Es gilt für **beide** Wege ins Fenster (Programmstart und Sperren aus der laufenden
  Sitzung) und ebenso für die kurze, nicht interaktive Sperr-Blende, die das WebView
  beim Sperren über die Aufgaben legt, damit
  beim Fensterwechsel keine Zeile aufblitzt, die drüben fehlt. **Ausnahme, und nur
  diese:** der Fehlerbildschirm N6 („Vault cannot be opened" samt Grund) behält seine
  Zeilen, sonst stünde der Nutzer vor einem stummen Fenster; das Layout meldet ihre
  Höhe an die gemeinsame Ring-Geometrie, damit der Ring auch dort mittig sitzt.
- **Kein „Unlock"-Knopf, und Luft um den Ring (verbindlich, Etikett N11.24,
  2026-08-08).** Entsperrt wird **allein mit Enter** im Passwortfeld (die Unterzeile,
  die das anfangs aussprach, ist mit N11.26 weggefallen). Grund ist nicht
  ...
  **und** zwischen Ring und dem ersten Steuerelement darunter (seit N11.26 ist das
  die Passwort-Pille) steht ein deutlich groesserer Abstand als
...
- **Keine Klartext-Anzeige, und die Pille bleibt eine Pille (verbindlich,
  ...
  2. **Kleiner und wirklich pillenförmig.** Das Feld ist 240x40 statt 320x46
     ...
     Formularzeile. **Der Rand liegt auf ganzen Pixeln (verbindlich):** eine
     gebrochene Strichstärke (der Fokusrand war 1,4 px) auf einer halben
     Pixelgrenze verteilt GDI+ über zwei Pixelreihen; auf den Geraden bleibt
     die Linie kräftig, in den Rundungen wird sie blass und breit, und die
     Enden sehen ausgefranst aus, also gerade das, was eine Pille ausmacht.
     Auf dem Sperrschirm hat das Feld **immer** den Fokus, man sah also immer
     den schlechteren Fall. Regel: ganzzahlige Strichstärke (mit der
     Bildschirmskalierung mitwachsend, `wintheme.line_w`) und ein Pfad, der um
     genau die halbe Strichstärke nach innen liegt (`wintheme.stroke_pill`,
     die eine Quelle für alle gemalten Pillen, Eingaben wie Knöpfe). Wer eine
     gemalte Pille neu baut, benutzt sie; freihändige Halb-Pixel-Masse bringen
     den ausgefransten Rand zurück. Die Höhe steckt in der gemeinsamen Ring-Geometrie
...
- **Das Schloss geht auf (verbindlich, Etikett N11.22, 2026-08-08).** Eine richtige
  ...
  sein rechtes Standbein nach oben weg, der Ring gibt einen kleinen Pulsschlag, und
  alles andere (Passwortfeld, Off-Knopf, Reset-Fusszeile) wird ausgeblendet (im
  Fehlerbildschirm N6, der als einziger noch Textzeilen hat, ersetzt „Unlocked" sie).
  Verbindlich daran: **kurz** (rund 0,6 Sekunden,
  ...
  nichts auf die Animation).
- **Genau eine Feier, nie zwei (verbindlich, Etikett N11.26, 2026-08-08).** Das
  aufgehende Schloss läuft **nur bei der Rückkehr in eine laufende, zwischendurch
  gesperrte Sitzung** (`resumed = true`). Beim **normalen Programmstart** bleibt das
  Schloss zu, das Fenster schliesst nach dem erfolgreichen Entsperren sofort, und die
  Begrüssung übernimmt drüben die Willkommens-Blende mit der Logo-Marke (N11.20);
  die Übergabe-Blende (N11.25) trägt auf diesem Weg deshalb **kein** Bild
  (Zustand `plain`), sonst käme das offene Schloss durch die Hintertür zurück. Damit
  gilt in beide Richtungen: entweder das Schloss oder das Logo, nie beides
  ...
  Sperrfensters durch.
- **Erst der Ring, dann zählt der Klick: der Off-Knopf (verbindlich, Etikett
  N11.28, 2026-08-10).** Der Power-Knopf oben rechts (N10.2) beendet die App nicht
  mehr beim ersten Klick. Sechs Punkte, alle verbindlich:
  1. **Ausgelöst wird mit dem Zeiger.** Steht die Maus auf dem Knopf, fährt der
     Kreis um das Power-Zeichen **in der Akzentfarbe** von oben im Uhrzeigersinn
     herum (rund 1,1 Sekunden); die Fläche wärmt sich dabei leicht an.
  2. **Der Lauf gehört sich selbst.** Fährt der Zeiger mittendrin woanders hin,
     läuft der Ring trotzdem zu Ende. Er springt nicht zurück, und ein erneutes
     Berühren startet ihn nicht neu.
  3. **Voller Kreis heisst scharf.** Dann, und **erst dann**, wird das
     Power-Zeichen selbst farbig, und ein Klick beendet die App über den
     unveränderten Weg (`quit_app()` → `teardown('quit')`, G35).
  4. **Vorher ist der Knopf stumm.** Ein Klick auf den nicht fertigen Ring tut
     **nichts**: kein Beenden, keine Meldung (der Ring sagt bereits, was fehlt).
     Damit ist der zweite Ausgang der App gegen den versehentlichen Klick
     gesichert, was er vorher nicht war.
  5. **Der scharfe Zustand hält 10 Sekunden.** Bleibt der Klick aus, fällt der
     Knopf in den Ruhezustand zurück (Ring weg, Zeichen wieder gedämpft); der
     Zeiger darauf löst ihn neu aus, auch durch blosse Bewegung, falls er nach
     Ablauf noch darauf liegt. Ein einmal berührter Knopf soll nicht den ganzen
     Abend scharf dastehen.
  6. **Kein eigener Hover-Farbwechsel mehr.** Das frühere Umschlagen in die
     Warnfarbe beim Darüberfahren fällt weg; die Hover-Rückmeldung **ist** der
     Ring, ein zweiter Farbsprung würde sie übertönen.

  Der Ring hält nichts auf, was schützt (der Tresor ist während der Sperre ohnehin
  zu, das Wischen der Spuren läuft erst nach dem Klick los), und solange das
  Beenden läuft, nimmt das Fenster weder einen Entsperr-Versuch noch den Reset an,
  damit nie zwei Ausgänge nebeneinander laufen. Der Ring nimmt den **festen**
  Akzent der nativen Palette (B.3, Punkt 1): die vom Nutzer eingestellte
  Akzentfarbe liegt im geschlossenen Tresor und ist während der Sperre nicht
  lesbar, genau wie beim grossen Sperr-Ring daneben.
...
*(Verfeinert durch N11.28, 2026-08-10, siehe die Sperrschirm-Liste oben: „ein Klick
beendet sofort" gilt seitdem erst für den Klick auf den **fertigen** Ring. Der Zeiger
auf dem Knopf lässt den Ring herumfahren (rund 0,75 s), vorher ist der Knopf stumm.
Am Weg selbst und an der Regel „löscht nie Daten" ändert das nichts.)*
...
> |---|---|---|---|---|---|
> | **G6** | **8** | ✅ erledigt (2026-07-21); dokumentierte N11.18-Ausnahme: die Arbeitsdatei ist SQLCipher-verschluesselt, nie Klartext | 2026-07-21 | Im entsperrten Betrieb existiert zu keinem Zeitpunkt eine entschlüsselte DB-Datei auf der Platte (Datei-Monitor auf `%TEMP%` und den App-Ordner während Unlock, Arbeit und Lock). | **In-Memory-DB** (`:memory:`) statt entschlüsselter Temp-Arbeitskopie, siehe B.7 „Alternative für Puristen". Eliminiert Temp-Datei-Forensik (Secure-Delete auf SSD ist unzuverlässig). |
> | **G7** | **8** | ✅ erledigt (2026-07-21); Regressionstest `tests/test_db.py` (2026-08-10) | 2026-08-10 | `db.py` setzt `PRAGMA key` als Raw-Key mit 64 Hex-Zeichen; nirgends mehr ein String-Key oder eine `'%s'`-Interpolation. | **Roher Hex-Schlüssel** für `PRAGMA key = "x'<64 hex>'"` statt String-Interpolation (`db.py`), damit SQLCipher kein eigenes PBKDF2 über den schon abgeleiteten Key legt (und das Quote-Escaping entfällt). Den Dev-Platzhalter in `db.py` bei dieser Gelegenheit ersetzen. |
> | **G8** | **8** | ✅ erledigt (2026-07-21) | 2026-07-21 | Argon2id-Parameter im Code ablesen (Memory ≥ 256 MB, time_cost ≥ 3); das Setup akzeptiert 12 gleiche Zeichen als Passphrase, und die UI zeigt keinerlei Stärkemesser oder Zeichenregeln. | **Starke Argon2id-Parameter** (Memory ≥ 256-512 MB, time_cost ≥ 3) **plus die Passphrase-Politik aus N11.3 (B.2): ausschliesslich Mindestlänge 12 Zeichen, kein Stärkemesser, keine Zeichenregeln.** Die Passphrase ist der einzige reale Schwachpunkt (Offline-Brute-Force), abgefedert wird das allein über die Argon2id-Kosten und den Pepper; das ist wichtiger als die zweite Cipher-Schicht. Frühere Fassungen verlangten eine „erzwungene Passphrase-Stärke mit Stärke-Anzeige"; das ist bewusst gestrichen und darf nicht wieder eingebaut werden (ehrliche Konsequenz: `aaaaaaaaaaaa` ist gültig). |
> | **🔴 G9** | **8** | ✅ erledigt (2026-07-21); Regressionstest `tests/test_release.py` (2026-08-10) | 2026-08-10 | `grep DEV_AES_KEY` über `src/` liefert 0 Treffer; `db.connect()` ohne passphrase-abgeleiteten Schlüssel schlägt fehl. | **`DEV_AES_KEY` & jeden statischen Schlüssel-Default ersatzlos entfernen.** Es darf **keinen** Code-Pfad geben, der die DB ohne passphrase-abgeleiteten Schlüssel öffnet. Sonst öffnet die „verschlüsselte" DB mit einem öffentlich im Quellcode stehenden String → **effektiv null Verschlüsselung**, während der Status fälschlich „AES-256 + ChaCha20" meldet. Wichtigstes Gate der Phase 8. Dazu gehören der saubere Erst-Einrichtungs-Flow (Passphrase anlegen) und die Migration der bestehenden Dev-DB auf den echten Schlüssel. |
> | **G11** | **0 / 9 (Build)** | ✅ vollständig erfüllt: Pinning über `requirements.lock.txt`, Hash-Checking über `requirements.lock.hashes.txt` (2026-08-10), Interpreter-Pin im Build erzwungen | 2026-08-10 | Jede Zeile in `requirements.lock.txt` trägt eine feste `==`-Version; `requirements.lock.hashes.txt` trägt zusätzlich zu jeder Zeile mindestens einen `--hash=sha256:`, erzeugt von `tools/lock_hashes.py`, und `pip install --require-hashes -r requirements.lock.hashes.txt` läuft ohne Beanstandung durch (nachgewiesen 2026-08-10 als Trockenlauf über alle 27 Pakete); `tools/build_exe.py` bricht ab, wenn nicht CPython 3.11.x läuft. | **Abhängigkeiten pinnen.** Die verbindliche gepinnte Menge ist `requirements.lock.txt` (liegt vor; `requirements.txt` bleibt bewusst die lose Liste des Direktbedarfs; frühere Fassungen dieses Gates verlangten das Pinning fälschlich in `requirements.txt` selbst). Rest-Pflicht in Phase 9: der Release-Build installiert nur aus der Lock-Datei, mit `pip` Hash-Checking. Eine getauschte Lib = Totalkompromittierung der Tresor-App. **Auch der Interpreter ist eine gepinnte Abhaengigkeit (U25):** die Ziel-Python-Version ist **3.11.x** (heutiges Setup: Microsoft-Store-Python 3.11), festgehalten in der Doku und in der Build-Umgebung (Phase 9), weil `sqlcipher3-wheels` Wheels nur fuer bestimmte CPython-Versionen liefert (eine falsche Version = kein passendes Wheel = stiller Bruch) und die `.exe` gegen genau diesen Interpreter gebaut werden muss. Gilt laufend: bei jedem Dependency-Update bewusst prüfen. |
...
> **Stand der Ablage seit 2026-08-14 (Register N11.32, Vorbereitung der Veröffentlichung).**
> Der Quellordner heisst **`src/`** (vorher `Code/`), das Logo liegt als
> `assets/noatodo-logo.png` in der Wurzel, und **`CLAUDE.md` sowie dieser
> `Planung/`-Ordner sind nicht mehr im Repository getrackt**: sie stehen in
> `.gitignore` und leben nur noch auf der Platte des Entwicklers. An der Rangfolge
> ändert das nichts, dieser Bauplan bleibt das verbindliche Dokument. Es heisst nur,
> dass er nicht mitveröffentlicht wird; ein verlorener Arbeitsstand kommt mit
> `git show <Commit vor dem Untracking>:"Planung/Bauplan - NoaToDo.md"` zurück.
> Ein `data/`-Ordner entsteht seit Phase 8 **nicht** mehr (der Tresor liegt am vom
> Nutzer gewählten Ort, siehe N11.3); die Zeile unten ist der Stand von Phase 0.

**Tun:**
1. Struktur anlegen (im `src/`-Ordner des Projekts):
   ```
   src/
   ├── main.py
   ├── requirements.txt
   ...
   └── data/                 # tasks.db entsteht hier automatisch (bis Phase 8)
   ```
...
> B.9 (Regel aus Plananalyse S1). Stand 2026-08-10: vollständig erfüllt,
> `requirements.lock.txt` (Versionen) plus `requirements.lock.hashes.txt`
> (Artefakt-Hashes, `--require-hashes`).
...
> Lock-Fenster `src/lockwindow.py` (Spike-Ergebnis N11.18), die Boot-Schleife in
...
> Die beiden letzten UX-Ergänzungen der Setup-UI sind am 2026-08-08 nachgezogen: die
> BitLocker-Empfehlung auf Onboarding-Schritt 1 (G31, mit dem realen Status des gewählten
> Laufwerks und einer eigenen Fassung für den nicht lesbaren Fall) und der
> quittierpflichtige SSD-Restgrenzen-Einmalhinweis als Schritt 4 (G33, nur wenn wirklich
> eine alte Dev-DB entsorgt wurde). Damit sind alle Phase-8-Gates vollständig. Real verifiziert
...
> Akzent-Pille, Reset-Fusszeile). Bausteine in `src/wintheme.py` (dort auch das
...
**Spike-Ergebnis (2026-07-21, ausgefuehrt als `src/tools/spike_u3_lockwindow.py`, Register N11.18):**
...
##### N11.30 Stand der Testbasis (umgesetzt 2026-08-10)

`src/tests/` existiert, `pytest.ini` liegt daneben, Aufruf aus `src/`:
`.\venv\Scripts\python.exe -m pytest` (**63 Tests, grün**, rund 4 Sekunden).

**Sicherheitsregel dieser Testbasis (bindend):** `tests/conftest.py` biegt vor
**jedem** Test alles weg, was echte Nutzerdaten wären: `%LOCALAPPDATA%` (und damit
`config.json` und der Arbeitsordner) auf einen Temp-Ordner, `config_path()`
ausdrücklich mit, und der DPAPI-Pepper auf einen festen Testwert.
`keyring.get/set/delete_password` werfen im Test eine `AssertionError`: ein Test,
der den echten Credential Manager anfasst, könnte den Pepper des Nutzers löschen
und dessen Tresor unwiderruflich unlesbar machen. Argon2id läuft mit 64 MiB / t=1,
also am unteren Rand des **erlaubten** Bereichs (N11.4.3), nie ausserhalb.

| Datei | Deckt ab |
|---|---|
| `test_db.py` | leerer Erststart (N11.1.4), CRUD, Abschnitts-Positionen (U13), `edit_task`-Spalten-Whitelist, `reorder`-Mengenprüfung, `move_task` behält `done`, parametrisierte Queries, Settings-Migration (N11.23), Arbeitsdatei ohne SQLite-Klartextkopf |
| `test_api_validation.py` | G20 (Kappung, Steuerzeichen, IDs, Settings-Whitelist samt Wertprüfung V5, `reorder_lists`, `move_task`-Randfälle), G21-Dateinamen, und die Festnagelung der **zwei** Bridge-Methoden ohne Argument-Schema (`set_setting` prüft je Schlüssel, `unlock` darf die Passphrase nicht antasten) |
| `test_lock_allowlist.py` | G13 dynamisch über alle Bridge-Methoden, `get_state()` gesperrt nur `{locked:true}`, Allowlist lässt wirklich durch und ist genau die vereinbarte Menge |
| `test_crypto.py` | G15 (Domänentrennung, impliziter Passphrase-Beweis), G18 (anderer Pepper öffnet nicht), G16 (Magic, frische Nonce, Kopf als AAD, Parameterbereich, `.bak`-Rettung), G28 (kein Klartext im Arbeits-Artefakt), N11.3/U8c (Passphrase-Wechsel macht `.enc` **und** `.bak` alt-unlesbar) |
| `test_ratelimit.py` | B.8.4/N11.4.1: Stufenfunktion samt Deckel, Persistenz über Neustart, Persist-before-verify, nur Erfolg räumt auf, `undo_last_fail`, zurückgedrehte Uhr verkürzt nichts, `config.json` ohne Geheimnisse |
| `test_teardown_killswitch.py` | G35 (genau eine Routine), Killswitch entfernt `.enc`/`.bak`/Konfig und der Folgestart ist Onboarding, G25 (Schlüssel genullt), Gegenprobe Sperren vernichtet nichts, Arbeitsdatei nach dem Sperren weg |
| `test_release.py` | G34 (Debug im Release hart aus, kein zweiter Debug-Schalter, `text_select=False`, die drei WebView-Schalter, Spec mit `optimize=2` ohne UPX), G27 (Manifest, keine Prüfung ohne Manifest, geändertes Byte und fehlende Datei fallen auf), G29 (kein `FileHandler`/`basicConfig(filename=)`/faulthandler-Ziel, Fehlercode-Katalog = B.2, Ringpuffer redigiert), CSP, `esc()`-Regel, und vier Regressionen (`DEV_AES_KEY`, `SetWindowDisplayAffinity`, `private_mode=True`, Win+L-Hook) |

**Ehrliche Lücken (bewusst, nicht vergessen):** Der XSS-Test ist die
**Trockenprüfung** (keine Interpolation eines Fremdwerts ohne `esc()` in
`app.js`), nicht der laufende DOM-Test: dafür bräuchte es den CDP-Weg mit
laufendem Fenster, der weiter der Handgriff aus der Dev-Doku bleibt. Ebenso
ungetestet automatisiert: alles, was ein echtes Fenster braucht (G6-Dateimonitor,
G17-Hartabbruch, G19-Zweitinstanz, native Dialoge, Auto-Sperre in Echtzeit) sowie
die G34-Tastenprüfungen. Dafür ist die folgende Checkliste da.

##### N11.31 Manuelle Smoke- und Abnahme-Checkliste (Phase 9)

Am Stück durchzugehen, mit der **gebauten `NoaToDo.exe`** (nicht aus dem
Quellbaum), am besten einmal auf dem Entwicklungsrechner und einmal auf einem
frischen Windows-Profil:

1. **Erststart ohne Tresor:** Onboarding erscheint (kein leeres/weisses Fenster),
   Speicherort wählbar, BitLocker-Hinweis passt zum Laufwerk (G31a), Passphrase
   unter 12 Zeichen wird abgelehnt, Verlust-Warnung ist quittierpflichtig.
2. **Grundfunktionen:** Liste anlegen, Aufgabe anlegen, abhaken (Ton), Doppelklick
   = Inline-Edit, Drag = Umsortieren, Rechtsklick = "Move to…", Export (`Ctrl+E`)
   schreibt eine Datei, Mini-Modus an/aus.
3. **Sperren/Entsperren:** `Ctrl+L` sperrt, der Wechsel ist nahtlos (N11.25, kein
   leerer Bildschirm, kein Minimieren), falsche Passphrase wartet 2 s, richtige
   öffnet das Schloss (N11.22) und die Liste steht wieder da.
4. **Off-Knopf:** Zeiger auf den Power-Knopf lässt den Ring laufen, ein Klick
   **vorher** tut nichts, nach dem vollen Kreis beendet er die App (N11.28).
5. **G34 am laufenden Fenster:** `NOATODO_DEBUG=1` gesetzt starten: F12 tot,
   Rechtsklick zeigt **kein** Browser-Kontextmenü, `Ctrl+P` öffnet **keinen**
   Druckdialog, Aufgabentext lässt sich nicht markieren, Eingabefelder schon.
6. **Status-Modal:** zeigt Version + Build-Datum, "Code signature: not signed",
   beide Verschlüsselungs-Schichten, den realen BitLocker-Zustand, WebView2-Version.
7. **Nach dem Beenden:** kein `msedgewebview2.exe` mit NoaToDo-Profil übrig, der
   Profilordner unter `%LOCALAPPDATA%\NoaToDo\webview` ist weg (G14), keine
   Log- oder Absturzdatei entstanden (G29).
8. **Fremder Rechner:** `.exe` kopieren, starten: SmartScreen-Warnung erscheint
   (unsigniert, N11.29 b, erwartet), danach Onboarding mit **neuem** Tresor; eine
   mitkopierte `tasks.db.enc` lässt sich dort **nicht** öffnen (fehlender
   DPAPI-Pepper, G18) und meldet das als Fehlerbildschirm, nicht als Absturz.
...
2. **Build / Packaging (Bauprozess):**
   ...
   - *(Umgesetzt 2026-08-10: `src/requirements.lock.hashes.txt`, erzeugt von
     `src/tools/lock_hashes.py`; Installation für den Release-Build mit
...
- **(a2) So wird gebaut (umgesetzt 2026-08-10).** Ein Einstiegspunkt:
  `src/tools/build_exe.py`, aufgerufen mit dem venv-Python des Projekts, oder
  bequem per Doppelklick über `src/build.bat` (das Gegenstück zu `run.ps1`
  ...
  Bau-Stempel `src/_buildstamp.py` (Build-Datum, Commit samt `+dirty`-Kennung,
  `SIGNED = False`, das SHA-256-Frontend-Manifest für G27), (3) schreibt die
  Versions-Ressource `src/build/version_info.txt`, (4) ruft PyInstaller mit
  `src/NoaToDo.spec` auf und (5) **löscht den Stempel wieder** (bliebe er
  ...
  den Windows-Eigenschaften. `src/requirements-dev.txt` pinnt die
...
- **(d) Version und Bezugsquelle.** Erste ausgelieferte Fassung ist **1.0.0**;
  Version, Build-Datum und Bezugsquelle stehen in `src/buildinfo.py` (eine
...
6. **Update-/Release-Story (V10, ergänzt 2026-07-15; Status-Modal-Teil umgesetzt 2026-08-10):**
   - **Version sichtbar:** das Status-Modal zeigt Versionsnummer und Build-Datum an
     (dieselben Werte, die in die `.exe`-Ressourcen geschrieben werden). *Umgesetzt:
     `get_status().app` liefert `{version, build_date, commit, frozen, signed, source}`
     aus `src/buildinfo.py`, das Modal rendert daraus die drei Zeilen „Version",
     „Code signature" und „Updates". Die Signatur-Zeile steht ausdrücklich dabei,
     damit die unsignierte Fassung (N11.29 b) sich nicht als signiert liest (G22).*
...
| N11.18 / U3-Ergebnis | 2026-07-21 | Zweitprofil-Spike ausgefuehrt (`src/tools/spike_u3_lockwindow.py`): kein Zwei-Profil-Beweis moeglich, **nativer Lock-Fenster-Fallback verbindlich** (kein `LOCK_PROFILE_DIR`); WebView-Abbau/Neuaufbau im selben Prozess samt G14-Wischbarkeit bewiesen; `setup_app()`-Vorbedingung dokumentiert; G6-Nebenbefund: `sqlcipher3` ohne `serialize` -> N11.9-Arbeitsdatei-Fallback (Schnappschuss via `VACUUM INTO`) verbindlich | Phase 8 (Spike-Ergebnis) |
...
| N11.32 | 2026-08-14 | **Das Repository wird auf Veröffentlichung vorbereitet** (Nutzerentscheid): Der Quellordner heisst **`src/`** statt `Code/` (alle Pfade im Code sind relativ, betroffen war nur die Prosa), das Logo liegt als `assets/noatodo-logo.png` in der Wurzel, und **`CLAUDE.md` sowie `Planung/` sind nicht mehr getrackt** (`git rm --cached` + `.gitignore`), bleiben aber lokal bestehen und behalten ihren Rang: der Bauplan ist weiter das verbindliche Dokument, er wird nur nicht mitveröffentlicht. Mitgeräumt: `tools/make_icon.py` zeigte noch auf `Planung/NoaToDo Logo.png` und lief deshalb seit dem Umsortieren gar nicht mehr; die einzige deutsche Meldung an einen Nutzer (Notweg der Einzelinstanz-Meldung, `main.py`) ist jetzt englisch wie die ganze Oberfläche; `pillow` steht nur noch in `requirements-dev.txt` und in den Spec-`excludes`, weil die App selbst kein PIL importiert (es wird allein von `make_icon.py` gebraucht). **Offen und bewusst noch nicht entschieden:** Lizenz, `LICENSE`, Lizenzkopfzeilen, die OFL-Pflichtvermerke der beiden Schriften und `README`/`SECURITY.md`. Bis dahin bleibt das Repository privat | Phase 0 (Ablage) + B.10.1 (Quellcode ist kein Schutzgut) |
...
Theme-folgt-Windows + Ton-UI (N11.6, 2026-08-08). Die letzten zwei UX-Ergaenzungen der
Setup-UI aus G31/G33 (BitLocker-Empfehlung, quittierpflichtiger SSD-Restgrenzen-Hinweis)
sind am 2026-08-08 nachgezogen, damit sind alle Phase-8-Gates vollstaendig. Es fehlt nur
noch Phase 9 (Build/Packaging). Die Haken fuer 0 bis 5
...
| ✅ G11 | erledigt (2026-08-10), Pflege läuft weiter | Abhängigkeiten versions-pinnen (`requirements.lock.txt`) + Artefakt-Hashes (`requirements.lock.hashes.txt`, `--require-hashes`) + Python 3.11.x im Build erzwungen (U25) |
...
| 🟡 G27 | 9, teils erledigt (2026-08-10) | Binary-Härtung: `.exe` signieren, kein Quelltext mitliefern (Nuitka), optional Obfuskation. Sicherheit beruht nie auf Code-Geheimhaltung (Kerckhoffs), nur auf Passphrase + Pepper + Verschlüsselung. **Ergänzung Frontend-Integrität (A5, 2026-07-15):** Assets ins signierte Binary einbetten oder Start-Hash-Prüfung gegen ein eingebettetes Manifest; Abweichung = Startabbruch mit klarer Meldung | *(Frontend-Integritaet und der Punkt 'kein Quelltext' sind erfuellt; Signing offen, kein Zertifikat, N11.29 b)*
| ✅ G28 | erledigt (2026-07-21); pytest-Fassung Rest 9 | Verschlüsselungs-Beweis (N11.9): erbracht in `src/tools/verify_crypto.py` (standalone, ohne pytest) und real gegen die echte `tasks.db.enc` verifiziert: kein SQLite-Klartext-Header (`SQLite format 3`), kein Task-Text im Roh-Byte-Dump, Magic `NOA1`; auch die Arbeitsdatei ist AES-Chiffretext. Die Verankerung als pytest (V12) kommt mit der Phase-9-Testliste |
...
| ✅ G31 | erledigt (2026-07-21), Anzeige 2026-07-25 | RAM-auf-Platte-Lecks (A1): realer BitLocker-Status per WMI (sonst ehrlich "unknown", nie falsches "protected"); backendseitig seit 2026-07-21, als eigene Zeile im Status-Modal **sichtbar** seit 2026-07-25, `VirtualLock` für alle Schlüssel-Puffer (Best-Effort; hilft nicht gegen `hiberfil.sys`), keine Traceback-/Dump-Dateien (deckt sich mit G29). **BitLocker-Empfehlung in der Einrichtungs-UI seit 2026-08-08:** Onboarding-Schritt 1 zeigt sie mit dem realen Status des gewählten Laufwerks (`choose_vault_dir().bitlocker`), und zwar mit **drei** Textfassungen, weil ein gemessenes "aus" und ein nicht lesbarer Status nicht dasselbe behaupten dürfen (G22): `protected` bestätigt die Deckung, `off` empfiehlt das Einschalten, `unknown` sagt ausdrücklich, dass die App es nicht weiss (die Abfrage braucht meist Adminrechte) und bittet den Nutzer, selbst nachzusehen |
...
| ✅ G33 | erledigt (2026-07-21) | Dev-Altdaten (A3): beim ersten `create_vault()` alte `data/tasks.db` samt `-journal`/`-wal`/`-shm` über den Secure-Delete-Pfad entsorgt (nie blankes `os.remove`). **Ehrlicher SSD-Restgrenzen-Hinweis seit 2026-08-08:** `_cleanup_dev_legacy_db()` meldet jetzt, ob wirklich etwas gefunden und entsorgt wurde (`create_vault().legacy_removed`); nur dann zeigt das Onboarding Schritt 4, einen quittierpflichtigen Einmal-Hinweis ("I have read this") vor der Willkommens-Blende, der beide Restkanäle nennt (SSD-Wear-Leveling macht das Überschreiben zu Best-Effort; alte Export-Dateien sind eigene Dateien des Nutzers, die die App bewusst nicht sucht und nicht löscht) und auf Vollverschlüsselung als einzige vollständige Antwort verweist |
| 🟡 G34 | 9, im Code erledigt (2026-08-10) | Release-Härtung (A4/A6, 2026-07-15): `NOATODO_DEBUG` im Release wirkungslos (Build-Konstante), DevTools aus (`AreDevToolsEnabled=false`), `AreBrowserAcceleratorKeysEnabled=false` (kein `Strg+P`-Klartext-PDF an G21 vorbei), Standard-Kontextmenü aus; **`text_select=False` explizit setzen + Regressionstest: SOFORT, Termin 2026-07-20**; Eingabefeld-Copy bleibt offener Kanal (B.10.3 Punkt 8) | *(Tastenpruefung am laufenden Fenster steht noch aus, Checkliste N11.31 Punkt 5)*
